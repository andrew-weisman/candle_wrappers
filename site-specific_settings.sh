...
if [ "x$SITE" == "xbiowulf" ]; then
    ...
    export CANDLE_POSSIBLE_KEYWORDS_AND_DEFAULTS="{'model_script': None, 'workflow': None, 'walltime': '00:05:00', 'worker_type': 'k80', 'nworkers': 1, 'nthreads': 1, 'custom_sbatch_args': '', 'mem_per_cpu': 7, 'dl_backend': 'keras', 'supp_modules': '', 'python_bin_path': ''}"
    ...
    # Note: A keyword is specified to be required by settings its default value to None
    export CANDLE_POSSIBLE_KEYWORDS_AND_DEFAULTS="{'model_script': None, 'workflow': None, 'walltime': '00:05', 'nworkers': 1, 'project': None, 'dl_backend': 'keras', 'supp_modules': '', 'python_bin_path': ''}"
